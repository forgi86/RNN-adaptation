...
if __name__ == '__main__':
    ...
    params = {"C": 500e-9, "L": 50e-6, "R": 4.5}

    def f_ODE(t, x):
        u = u_func(t).ravel()
        return fxu_ODE(t, x, u, params)


    def f_ODE_nl(t, x):
        u = u_func(t).ravel()
        return fxu_ODE_nl(t, x, u, params)
    ...
    df_X.to_csv(os.path.join("data", "RLC_data_transfer_lin.csv"), index=False)
    ...
    COL = COL_T + COL_X + COL_U + COL_Y
    df_X = pd.DataFrame(X, columns=COL)
    df_X.to_csv(os.path.join("data", "RLC_data_transfer_nl.csv"), index=False)
